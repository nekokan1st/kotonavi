...
export function AppChooser({ options }: { options: AppOption[] }) {
  ...
  return <section className="app-chooser" aria-labelledby="app-chooser-title">
    ...
    <div className="selected-app" aria-live="polite"><AppStoreVisual name={selected.name} variant="selected" /><div className="selected-app-copy"><small>{selected.kind}</small><h3>{selected.name}</h3><p>{selected.body}</p><dl><dt>向いているケース</dt><dd>{selected.fit}</dd><dt>利用前の注意</dt><dd>{selected.caution}</dd></dl><div className="selected-app-links">{!selectedIsStore && <a className="official-link" href={selected.url} target="_blank" rel="noreferrer">公式情報 <span>↗</span></a>}{ios && <a href={ios} target="_blank" rel="noreferrer">App Store <span>↗</span></a>}{android && <a href={android} target="_blank" rel="noreferrer">Google Play <span>↗</span></a>}</div></div></div>
  </section>;
}
