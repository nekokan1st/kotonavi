...
export default function ProblemsIndexClient() {
  ...
  return <main className="guide-page">
    <header className="topbar"><a className="brand" href="/"><span className="brand-mark"><i /><i /><i /></span><span>コトナビ</span></a><nav className="topnav"><a href="/">困りごとから探す</a><a href="/info">運営・掲載方針</a></nav></header>
    <section className="guide-hero problem-index-hero"><span className="overline">PROBLEM INDEX</span><h1>困りごとを、一覧から探す。</h1><p>すべての掲載テーマから、確認手順と使えるスマホアプリを探せます。言葉で探す場合は、トップの検索も使えます。</p><a className="guide-hero-link" href="/">トップで困りごとを検索する →</a></section>
    <section className="problem-index-controls" aria-label="困りごと一覧を絞り込む"><label><span>キーワード</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例：家計、薬、タクシー" /></label><div><span>テーマ</span><div className="problem-theme-filters">{themes.map((item) => <button className={theme === item ? "active" : ""} key={item} onClick={() => setTheme(item)} type="button">{item}</button>)}</div></div><p>{results.length}件の困りごと</p></section>
    <section className="guide-list problem-page-list">{results.map((item) => <article key={item.id}><small>{themeLabels[item.theme] ?? item.theme}</small><h2><a href={destinationFor(item.id)}>{item.title}</a></h2><p>{item.description}</p><a className="text-link" href={destinationFor(item.id)}>確認手順とアプリを見る →</a></article>)}{results.length === 0 && <p className="empty-problem-results">該当する困りごとがありません。別の言葉で探すか、トップからテーマを選んでください。</p>}</section>
