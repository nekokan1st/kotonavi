...
def mark(draw: ImageDraw.ImageDraw, x: int, y: int, size: int) -> None:
    ...
    draw.rounded_rectangle((x + half + gap, y, x + size, y + size * 38 // 100), radius=radius, fill=BLUE)
    draw.rounded_rectangle((x, y + half + gap, x + size * 38 // 100, y + size), radius=radius, fill=BLUE)
    draw.rounded_rectangle((x + size * 42 // 100, y + size * 42 // 100, x + size, y + size), radius=radius, fill=BLUE_PALE)
